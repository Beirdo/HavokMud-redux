...
${CLEOS} wallet unlock --password ${DEVELOPMENT_PASSWORD}
...
for account in ${PRODUCER_ACCOUNTS} ; do
  pubkey=$(cat ${KEY_DIR}/${account}.pubkey | sed -e 's/.*\"\(.*\)\".*/\1/')
  privkey=$(${BASE_DIR}/bin/extract_private_key.py ${KEY_DIR}/development_keys.json ${pubkey})
  rpc_port=${account/prod.1./800}
  p2p_port=${account/prod.1./901}
  # And start it up for real
  bash ${BASE_DIR}/bin/nodeos_start.sh ${account} ${pubkey} ${privkey} ${rpc_port} ${p2p_port}
done
